...
func (s *Server) GetTicket(w http.ResponseWriter, r *http.Request, ticketId int64) {
	...
	s.enrichTickets(ctx, []api.Ticket{ticket})
...
func (s *Server) GetTicketComments(w http.ResponseWriter, r *http.Request, ticketId int64) {
	ctx := r.Context()
	rows, err := s.queries.GetTicketComments(ctx, pgtype.Int8{Int64: ticketId, Valid: true})
	if err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
	...
	for _, row := range rows {
		c := api.Comment{
			Id:         row.ID,
			AuthorId:   pgint8Ptr(row.AuthorID),
			Body:       pgtextPtr(row.PlainBody),
			Public:     pgboolPtr(row.Public),
			ViaChannel: pgtextPtr(row.ViaChannel),
		}
